...
void createUi(HWND hwnd) {
    ...
    createSectionLabel(hwnd, 1009, L"CLO refinement v2.5.3 (two-pass VST CAB Tone Match - final 20 s)");
        ...
        if (r && r->ok) {
            ...
            if (!r->refinedAmpero2048.empty()) {
                resultMessage += L"\r\n\r\nBEST Block-B Ampero 2048 (VST tail tone-match candidate):\r\n" + r->bestAmpero2048.wstring()
                              ...
                              + L"\r\n\r\n--- v2.5.3 two-pass VST CAB Tone Match diagnostics (final 20 s) ---"
                              ...
                              + L"\r\n\r\nNote: v2.5.3 freezes PRE, Block A, P/K and POST. Pass 1 uses the SOURCE_latest_19 VST CAB Tone Match on the final 20 seconds at Smooth 5%, but removes global gain offset. Pass 2 optionally applies a small residual correction only from 1-8 kHz (max +/-1.5 dB), preserving the improved low-frequency match. Both use 2048-sample minimum-phase IRs before GP-200 compaction.";
            }
