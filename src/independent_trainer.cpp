...
// arguments and return values in float at those call sites. The direct-DFT
// trig-table builder is a separately audited exception: it promotes its float
// phase to double and uses double sin/cos before rounding back to float.
...
PK fitPk(const std::vector<float>& in,const std::vector<float>& out,double sr){
    ...
    // HTUSBTools.dll 0x18009e2e6 calls CRT floorf() on the float quotient,
    // then cvttss2si.  Do not promote to double/floor here.
    const int windows=static_cast<int>(::floorf(quotient));
...
void lowSmoothASequentialF(std::vector<float>&m,double sr){
    ...
    // HTUSBTools.dll 0x1800968c4 calls CRT floorf() on this float value,
    // followed by cvttss2si.  Preserve the float CRT boundary.
    const int limI=static_cast<int>(::floorf(raw));
    if(limI<=1)return;
...
TrigTableF makeDirectTrigOfficial(std::size_t n){
    ...
    // HTUSBTools.dll 0x18009598a..0x180095ac8: delta and phase are
    // calculated in float.  The phase is then promoted to double and the
    // first half is evaluated with DOUBLE sin/cos (scalar CRT for the tail;
    // the optimized pair loop uses the compiler's double-vector helpers),
    // followed by one double->float conversion.  This is intentionally not
    // sinf/cosf.  The second half is generated by XORing the float sign bit.
    ...
    for(std::size_t i=0;i<half;++i){
        ...
        const double phaseD=static_cast<double>(phase);
        t.s[i]=static_cast<float>(::sin(phaseD));
        t.c[i]=static_cast<float>(::cos(phaseD));
    }
...
std::vector<float> frequencyWeightsF(double sr){
    ...
    // HTUSBTools.dll 0x18009fcbc calls CRT floorf(), then cvttss2si; the
    // result is used as a one-based boundary.
    const int boundary=static_cast<int>(::floorf(raw));
